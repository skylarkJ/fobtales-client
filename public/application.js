...
/***/ function(module, exports, __webpack_require__) {
	...
	var onSignIn = function onSignIn(event) {
	  ...
	  api.signIn(data).then(function (response) {
	    ...
	    return store.user;
	  }).then(ui.signInSuccess).catch(ui.signInError);
	};
	...
	var onSignOut = function onSignOut(event) {
	  ...
	  api.signOut(data).then(function () {
	    delete store.user;
	    return store;
	  }).then(ui.signOutSuccess).catch(ui.signOutError);
	};

	var getStories = function getStories() {
	  api.stories().then(function (response) {
	    store.stories = response.stories;
	    return response;
	  }).then(ui.stories);
	};

	var createStory = function createStory(event) {
	  event.preventDefault();

	  var data = getFormFields(event.target);

	  api.createStory(data).then(getStories);
	};

	var updateStory = function updateStory(event) {
	  event.preventDefault();

	  var data = getFormFields(event.target);

	  api.updateStory(data.story.id, data).then(getStories).then(function () {
	    $('#modalUpdateStory').modal('hide');
	  });
	};

	var deleteStory = function deleteStory(event) {
	  event.preventDefault();

	  var data = getFormFields(event.target);

	  api.deleteStory(data.story.id).then(getStories).then(function () {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var updateStory = function updateStory(ID, data) {
	  return $.ajax({
...
/***/ function(module, exports, __webpack_require__) {
	...
	var clearModalInput = function clearModalInput(e) {
	...
	var clickOnStory = function clickOnStory(event) {
	  var id = $(event.target).data('id');
	  var story = store.stories.find(function (s) {
	    return s.id === id;
	  });
	...
	var stories = function stories(data) {
	  $('.stories').empty();
	...
	module.exports = {
	  ...
	  stories: stories,
	  clearModalInput: clearModalInput
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
